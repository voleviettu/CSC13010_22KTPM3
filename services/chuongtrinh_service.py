...
class ChuongTrinhService:
  ...
  def _nhap_ten_chuong_trinh(self, ten_chuong_trinh_cu=None):
    while True:
      if ten_chuong_trinh_cu:
        ten_chuong_trinh = input(f"Nhập tên chương trình đào tạo mới (hoặc ấn Enter để giữ nguyên '{ten_chuong_trinh_cu}'): ") or ten_chuong_trinh_cu
      else:
      ...
      if ten_chuong_trinh not in self.danh_sach_chuong_trinh:
        ...
        print(f"Tên chương trình đào tạo '{ten_chuong_trinh}' đã tồn tại. Vui lòng nhập tên khác.")

  def _chon_chuong_trinh(self):
    self.hien_thi_danh_sach_chuong_trinh()
    while True:
      ten_chuong_trinh = input("Nhập tên chương trình đào tạo bạn muốn chỉnh sửa: ")
